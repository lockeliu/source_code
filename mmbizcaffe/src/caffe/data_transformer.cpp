...
template<typename Dtype>
void DataTransformer<Dtype>::Transform_AugOnFly(const cv::Mat& img,
                                       Blob<Dtype>* transformed_blob) {
  cv::Mat cv_img;
  img.copyTo(cv_img);									   
  const int crop_size = param_.crop_size();
  const bool display = param_.display();
  const bool do_contrast_adjustment = param_.contrast_adjustment() && Rand(2);
  const bool do_smooth_filtering = param_.smooth_filtering() && Rand(2);
  const bool do_jpeg_compression = param_.jpeg_compression() && Rand(2);
  const bool do_rotation = param_.rotation() && Rand(2);
  ...
  if (crop_size) {
    ...
    cv_cropped_img.copyTo(cv_img); //breeze
    //if (display && phase_ == TRAIN)
  ...
  // Rotation------------------------------------------------------------breeze
  const float min_rotation_angle = param_.min_rotation_angle();
  const float max_rotation_angle = param_.max_rotation_angle();
  double rotation_angle = 0;
  if (do_rotation) {
  cv::Mat dst;
  rotation_angle = Rand(int(max_rotation_angle - min_rotation_angle)) + min_rotation_angle;
  ...
  cv::Mat r = getRotationMatrix2D(pt, rotation_angle, 1.0);
  ...
  if (display && phase_ == TRAIN) {
	...
	if ( do_smooth_filtering ) {
	  ...
	  LOG(INFO) << " smooth param1: " << smooth_param1;
	}
	if ( do_contrast_adjustment ) {
	...
	LOG(INFO) << "  min_rotation_angle: " << min_rotation_angle;
  ...
  for (int h = 0; h < height; ++h) {
    ...
    for (int w = 0; w < width; ++w) {
      for (int c = 0; c < img_channels; ++c) {
        ...
        if (has_mean_file) {
          int mean_index = (c * img_height + h + h_off) * img_width + w + w_off;
          transformed_data[top_index] =
